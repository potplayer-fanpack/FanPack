...
Name: "icaros";      Description: "{cm:comp_icaros}";      Types: custom; Check: IsIcarosInstalled; ExtraDiskSpaceRequired: 13201408
...
Source: "InstallDir\reg\delete_pot_progs_hkcu.reg";                            DestDir: "{app}";                                                Components: "program"; Flags: ignoreversion
...
Source: "InstallDir\reg\delete_icaros.reg";                                    DestDir: "{app}";                                                Components: "icaros"; Flags: ignoreversion
...
Filename: "reg"; Parameters: "IMPORT delete_pot_progs_hkcu.reg /reg:32";     WorkingDir: {app};  Flags: waituntilterminated runhidden runascurrentuser shellexec
...
begin
  Result := '';
  RegKey := 'Software\Microsoft\Windows\CurrentVersion\Uninstall\FanPack64_is1';
  if RegQueryStringValue(HKLM, RegKey, 'DisplayVersion', Version) then
  begin
    Result := Version;
  end;
end;
...
var
  InstalledVersion: String;
begin
  // Najpierw sprawdzamy, czy jest PotPlayer
  if not IsPotPlayerInstalled then
  begin
    MsgBox('Wymagany program PotPlayer nie został wykryty na tym komputerze. Zainstaluj PotPlayer, by kontynuować.', mbInformation, MB_OK);
    Result := False;
    Exit;
  end;
  // Sprawdzenie wersji zainstalowanej aplikacji (FanPack)
  InstalledVersion := GetInstalledVersion;
  if InstalledVersion <> '' then
  begin
    if CompareVersion(InstalledVersion, '{#MyAppVersion}') >= 0 then
    begin
      MsgBox('Wersja ' + InstalledVersion + ' lub nowsza jest już zainstalowana. Instalacja zostanie przerwana.', mbInformation, MB_OK);
      Result := False;
    end
    else
    begin
      if MsgBox('Wykryto zainstalowaną wersję ' + InstalledVersion + '. Czy chcesz zaktualizować do wersji {#MyAppVersion}?', mbConfirmation, MB_YESNO) = IDYES then
        Result := True
      else
        Result := False;
    end;
...
begin
  Result := not (RegKeyExists(HKEY_LOCAL_MACHINE, 'SOFTWARE\Icaros') or
...
var
  ResultCode: Integer;
begin
  Result := Exec(ExpandConstant('{tmp}\7za.exe'), 'x "' + ExpandConstant('{tmp}\Module64.7z') + '" -o"' + ExpandConstant('{autopf}\DAUM\PotPlayer\Module') + '" * -r -aoa', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  if ResultCode <> 0 then
  begin
    Log('7za.exe failed with code: ' + IntToStr(ResultCode));
    MsgBox('Failed to extract Module64.7z. Error code: ' + IntToStr(ResultCode), mbError, MB_OK);
    Result := False;
  end;
end;
...
begin
  if CurPageID = wpReady then begin
    // Sprawdzamy, czy komponent 'icaros' został wybrany
    if WizardIsComponentSelected('icaros') then begin
      DownloadPage.Clear;
      // Dodajemy pliki do pobrania
      ...
      // DownloadPage.Add('https://jrsoftware.org/download.php/iscrypt.dll?dontcount=1', 'Icaros.exe', '608ff4b0508f31e3d85810141cbb56b57304a385fc26cce8a9b4b2ad95c99c64');
      DownloadPage.Show;
      try
        try
          DownloadPage.Download; // Pobieranie plików do {tmp}
          Result := True;
        except
      ...
      Result := True; // Jeśli komponent nie jest wybrany, kontynuuj instalację bez pobierania
  end else
    Result := True;
end;
...
begin
  ...
  for I := 0 to GetArrayLength(TempFiles) - 1 do
    if FileExists(TempFiles[I]) then
      if not DeleteFile(TempFiles[I]) then
        Log('Failed to delete temporary file: ' + TempFiles[I]);
end;
