...
if not defined num (
	...
	yt-dlp.exe -f "bestvideo[height<=?1080][vcodec^=avc][ext=mp4]+bestaudio[acodec^=mp4a][ext=m4a]/best" "%url%" -P %DOWNLOADS% -o "%%(title).100s.%%(ext)s" --no-part --live-from-start
	if ERRORLEVEL 1 (goto check)
) else (
	yt-dlp.exe -f %num% "%url%" -P %DOWNLOADS% -o "%%(title).100s.%%(ext)s" --no-part --live-from-start
	if ERRORLEVEL 1 (goto check)
)
color 27
explorer %DOWNLOADS%
